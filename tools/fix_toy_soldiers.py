...
s = s.replace("let eliteClips = [];", "let standardClips = [];")

marker = "const audioCache = new Map();\n"
insert = """const audioCache = new Map();
...
"""
if marker not in s:
    raise SystemExit('audioCache marker missing')
...
old = """function playEnemyAnimation(enemy, name, loop = true, fade = 0.12) {
...
"""
new = """function playEnemyAnimation(enemy, name, loop = true, fade = 0.12) {
...
"""
if old not in s:
    raise SystemExit('playEnemyAnimation block missing')
s = s.replace(old, new)

start = s.index('function createEnemy(position, index = 0) {')
end = s.index('\nfunction spawnWave()', start)
new_create = """function createEnemy(position, index = 0) {
...
old = """  const clip = findClip('Death', enemy.clips || enemyGltf?.animations || []);
  if (clip) {
...
"""
new = """  enemy.deadAt = performance.now();
...
"""
if old not in s:
    raise SystemExit('death block missing')
s = s.replace(old, new)

old = """    enemy.hitTilt = side * (0.10 + Math.random() * 0.08);
    enemy.root.position.addScaledVector(shotDirection, 0.025 + Math.random() * 0.02);
"""
new = """    enemy.hitTilt = side * (0.10 + Math.random() * 0.08);
...
"""
if old not in s:
    raise SystemExit('hit push block missing')
s = s.replace(old, new)
...
"""
if old not in s:
    raise SystemExit('update loop start missing')
s = s.replace(old, new)
...
"""
new = """    ui.loadingText.textContent = 'تحميل جنود الألعاب...';
    enemyGltf = await loadGLTF(manifest.recommendedPrototype.enemy);
    enemyTemplate = enemyGltf.scene;
...
"""
if old not in s:
    raise SystemExit('init character block missing')
s = s.replace(old, new)
